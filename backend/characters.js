...
module.exports = function(options, utils) {
  ...
  //
  // Find by a name that starts with passed parameter.
  //
  internals.findNameStartsWith = function(name, limit, offset, fn) {
    var deferred = Q.defer()
      , ts = utils.timestamp();
  ...
  internals.series = function(id, limit, offset, fn) {
    ...
    request({
      url: util.format('http://gateway.marvel.com/v1/public/characters/%s/series', id)
    , json: true
    , qs: {
        ts: ts
      , apikey: pubkey
